...
class DataFrameIterator(Sequence):
    ...
    def __init__(self, dataframe, directory, image_data_generator,
        ...
        self.interpolation = interpolation
        self.dtype = dtype
        self.batch_size = batch_size  # Inicializando batch_size

        # Verificações
        if type(x_col) != str:
            raise ValueError("x_col must be a string.")
        if type(has_ext) != bool:
            raise ValueError("has_ext must be either True if filenames in x_col has extensions, else False.")
        ...
        if path_classes:
            ...
            self.classes = sorted(self.df[y_col].unique())
        ...
        self.samples = len(self.filenames)
        self.image_shape = (self.target_size[0], self.target_size[1], 3)  # Ajuste para RGB

        if self.nb_classes > 0:
            print(f'Found {self.samples} video folders belonging to {self.nb_classes} classes.')
        else:
            print(f'Found {self.samples} video folders.')
    ...
    def __getitem__(self, index):
        # Gera índices para o batch atual
        start = index * self.batch_size
        end = min((index + 1) * self.batch_size, self.samples)
        index_array = self.indexes[start:end]
        return self._get_batches_of_transformed_samples(index_array)

    def on_epoch_end(self):
        # Atualiza os índices ao final de cada época
        self.indexes = np.arange(self.samples)
        if self.shuffle:
            np.random.seed(self.seed)
            np.random.shuffle(self.indexes)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_x = np.zeros((len(index_array), self.nb_frames) + self.image_shape, dtype=self.dtype)
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                for idx in range(self.nb_frames):
                    ...
                    fname = f'{self.save_prefix}_{j}_{np.random.randint(1e7)}.{self.save_format}'
                    img.save(os.path.join(self.save_to_dir, fname))
        ...
        if self.class_mode == 'input':
            ...
            batch_y = self.df.iloc[index_array][self.y_col].values
        elif self.class_mode == 'binary':
            batch_y = self.df.iloc[index_array][self.y_col].values.astype(self.dtype)
        elif self.class_mode == 'categorical':
            batch_y = np.zeros((len(batch_x), self.nb_classes), dtype=self.dtype)
